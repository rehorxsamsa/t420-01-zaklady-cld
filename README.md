...
Z cheatsheetu: `npm install -g @anthropic-ai/claude-code`. To pořád platí. V roce 2026 máš dvě cesty:
...
Z cheatsheetu: `@` = Mention files/folders. Místo kopírování obsahu souboru do promptu na něj **odkážeš** přes `@`:

```
> Vysvětli mi, co dělá @src/Service/TaskService.php
```

```
> Projdi celou složku @src/Repository/ a zkontroluj, jestli jsou všechny SQL dotazy přes prepared statements
```

```
> Porovnej @src/Controller/TaskController.php se @src/Service/TaskService.php — neporušuje něco vrstvení?
```
...
```
> Mrkni na @src/Model/Task.php a @src/Repository/TaskRepository.php a navrhni, jak přidat pole `priority`
```
...
Z cheatsheetu: `!` = Bash mode prefix. Když řádek začneš `!`, Claude Code ho spustí jako shell příkaz (po tvém schválení) a výstup vidíš rovnou:
...
**Praktická ukázka `Ctrl+V`:** Rozbij něco v UI (např. v `templates/tasks.php` umaž `</div>`), vyfoť rozbitou stránku z prohlížeče a vlož screenshot přes `Ctrl+V`:
```
...
Z cheatsheetu: `Shift+Tab+Tab` = Plan mode. Claude **nejdřív navrhne plán** a teprve po tvém odsouhlasení něco udělá. Ideální na složité, vícekrokové změny:
...
Z cheatsheetu: `Shift+Tab` = Auto-accept ("yolo mode"). Claude přestane čekat na schválení každé editace a jede sám. Rychlé, ale **používej s rozmyslem** — hodí se na rutinu, kde Claude nemůže nic rozbít (formátování, generování boilerplate v izolované složce).
...
Z cheatsheetu: `/clear` = Clear history. Když dokončíš jeden úkol a jdeš na nesouvisející druhý, **vyčisti kontext**:
...
**3. Proč je lepší použít `@src/Service/TaskService.php` než zkopírovat obsah souboru do promptu?**
